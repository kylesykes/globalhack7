...
passport.use(
  new LocalStrategy({ usernameField: "phone" }, (phone, password, done) => {
    User.User.findOne({ phone: phone.toLowerCase() }, (err, user) => {
      if (err) {
        return done(err);
      }
      if (!user) {
...
passport.use(
  new FacebookStrategy(
    ...
    (req, accessToken, refreshToken, profile, done) => {
      if (req.user) {
        User.User.findOne({ facebook: profile.id }, (err, existingUser) => {
          if (err) {
            return done(err);
          }
          if (existingUser) {
...
passport.use(
  new GitHubStrategy(
    ...
    (req, accessToken, refreshToken, profile, done) => {
      if (req.user) {
        User.User.findOne({ github: profile.id }, (err, existingUser) => {
          if (existingUser) {
            req.flash("errors", {
              msg:
...
passport.use(
  new TwitterStrategy(
    ...
    (req, accessToken, tokenSecret, profile, done) => {
      if (req.user) {
        User.User.findOne({ twitter: profile.id }, (err, existingUser) => {
          if (err) {
            return done(err);
          }
          if (existingUser) {
...
passport.use(
  new GoogleStrategy(
    ...
    (req, accessToken, refreshToken, profile, done) => {
      if (req.user) {
        User.User.findOne({ google: profile.id }, (err, existingUser) => {
          if (err) {
            return done(err);
          }
          if (existingUser) {
...
passport.use(
  new LinkedInStrategy(
    ...
    (req, accessToken, refreshToken, profile, done) => {
      if (req.user) {
        User.User.findOne({ linkedin: profile.id }, (err, existingUser) => {
          if (err) {
            return done(err);
          }
          if (existingUser) {
...
passport.use(
  new InstagramStrategy(
    ...
    (req, accessToken, refreshToken, profile, done) => {
      if (req.user) {
        User.User.findOne({ instagram: profile.id }, (err, existingUser) => {
          if (err) {
            return done(err);
          }
          if (existingUser) {
...
passport.use(
  ...
  new OAuthStrategy(
    ...
    (req, token, tokenSecret, profile, done) => {
      User.User.findById(req.user._id, (err, user) => {
        if (err) {
          return done(err);
        }
...
passport.use(
  new OpenIDStrategy(
    ...
    (req, identifier, done) => {
      ...
      if (req.user) {
        User.User.findOne({ steam: steamId }, (err, existingUser) => {
          if (err) {
            return done(err);
          }
          if (existingUser) {
